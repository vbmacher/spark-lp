...
diagonally preconditioned CG iterations. [`strategy-results.json`](strategy-results.json) contains all three cases, including sparse payload/reference dense memory and timings.
...
[`comparison.csv`](comparison.csv). Repetition zero is retained and includes warmup effects. Repetition-one end-to-end timings:
...
explicit Cholesky is rejected. The [conditioning record](failed-cholesky-free.txt)
