...
Java 11.0.26, Java fallback BLAS/LAPACK, `local[4]`, four cached input partitions,
...
For an allocation run that previously crossed Auto's cutoff, explicitly select
`NewtonSolver.Cholesky` and set `SolveConfig.maxLocalConstraints` to the permitted
compiled row count. For example, a 10,000-row cap has an approximate 1.49 GiB
...
the resource cap alone does not override Auto's 1,000-row selection cutoff.

This evidence supports testing explicit block Cholesky on the EMR workload. It does
not prove the production runtime or establish a new universal Auto crossover.
