...
final class JvmSampler extends AutoCloseable {
  private val scheduler = Executors.newScheduledThreadPool(2, new ThreadFactory {
    override def newThread(r: Runnable): Thread = {
      val thread = new Thread(r, "benchmark-measurements"); thread.setDaemon(true); thread
    }
  })
  ...
  def reset(): Unit = { heap.set(0); rss.set(0); sample() }
  ...
  def watchdog(onTimeout: => Unit): AutoCloseable = {
    val scheduled = scheduler.schedule(new Runnable {
      override def run(): Unit = { onTimeout; Runtime.getRuntime.halt(124) }
    }, 30, TimeUnit.MINUTES)
    new AutoCloseable { override def close(): Unit = { scheduled.cancel(false); () } }
  }
  override def close(): Unit = { sampling.cancel(false); scheduler.shutdownNow(); () }
}
...
final class SolveMeasurements {
  ...
  def progress(event: SolveProgress): Unit = {
    ...
    if (name != phase) {
      ...
      phase = name; previous = event.elapsedSeconds
    }
