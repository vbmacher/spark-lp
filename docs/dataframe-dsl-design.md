...
Infeasibility is *not* detected in v1, and no claim is made about how an infeasible model fails: depending on the instance, the solver may stop at `IterationLimit` or abort with `LpNumericalException` when the iterates degenerate. The specific pair `x === 1` and `x === 2` is not an example of undetected infeasibility at all — the two rows have identical coefficients, so exact-duplicate row hashing (see "Open decisions — resolved") catches them and fails with an `LpModelException` naming both constraints as inconsistent duplicates. `LpStatus` deliberately has no `Infeasible`/`Unbounded` members yet, so no false claims can be encoded; they can be added compatibly when detection exists.
...
| `status` reported as optimal when the solver merely stopped | Solver-side `solveSummary` returning the convergence flag; `LpStatus` has no members the solver cannot truthfully report |
...
1. Add `LP.solveSummary` (iterations, convergence flag, final primal/dual/gap residuals) with the existing `solve` delegating to it, and wrap the numerical failure paths (`Initialize.init` inversion, per-iteration Cholesky, zero-iterate guard) in `LpNumericalException`; no behaviour change for successful current callers.
